    ...
    return (
        <Box height={`5vh`} borderBottom={`3px solid darkgray`} width={`100%`} paddingY={1} bgcolor={`#0b2431`} color={`white`}>
            <Box width={`95%`} marginX={`auto`} display={`flex`} alignItems={`center`}>
                ...
                <Avatar sx={{ marginLeft: 'auto', width: 36, height: 36, marginRight: 1 }} src={auth.currentUser.photoURL}>{auth.currentUser.displayName}</Avatar>
                <Typography variant="h6" fontWeight={500} marginRight={3}>{auth.currentUser.displayName}</Typography>
